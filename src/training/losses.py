...
def paired_infonce_loss(
    ...
    Note:
        论文写的是"参与损失计算的样本数量为 2B"，本实现用的是 ``[B, B]`` 交叉矩阵
        （每个锚点 ``B-1`` 个负样本）。**配对语义一致，负样本数量不同**——
        把 2B 个样本拼成一个矩阵时每个锚点会有 ``2B-2`` 个负样本，
        因为其它样本的锚点与正样本也都在同一行里。
        差异说明与"如何严格复现 2B 形式"见 ``docs/architecture.md`` §3.1.1。
