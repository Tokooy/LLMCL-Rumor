...
* `similarity[i, j] (j≠i)` 是论文所说的负样本 `(x_i, x'_j)`；
* 对称项 `similarity.T` 让 `z'` 也当一次锚点。

### 3.1.1 与论文"数量为 2B"的关系（一处需要说明的差异）

论文写的是""参与损失计算的样本包括原始样本及其增强样本，数量为 2B""。本实现在
**配对语义**上完全一致（对角为正、其余为负），但**负样本数量不同**：

| 方案 | 相似度矩阵 | 每个锚点的负样本数 |
|---|---|---|
| 本实现（``[B, B]`` 交叉矩阵 + 对角目标） | ``[B, B]`` | ``B − 1`` |
| 把 2B 个样本拼成一个矩阵 | ``[2B, 2B]`` | ``2B − 2`` |

之所以不同：把"原样本"和"增强样本"拼成 2B 长的序列时，其它样本的锚点与正样本
也都会进入同一行，因此负样本更多。两种写法都是 InfoNCE 的合法变体，
本实现选择 ``[B, B]`` 形式的原因是：

1. 它与论文"``(x_i, x'_i)`` 配对为正样本"的定义最直接对应（对角即配对）；
2. 显存与计算量是 ``O(B²)``，而 2B 形式是 ``O(4B²)``；
3. 需要屏蔽的"同一原样本的其它副本"在 ``[B, B]`` 形式里就是一次掩码操作，
   语义清晰（见 3.2）。

**若要严格复现 2B 形式**，只需把 anchor 与 positive 在 batch 维拼接后构造
``[2B, 2B]`` 相似度矩阵、遮蔽"同一样本的两行/两列"、目标设为 ``i`` 与 ``B+i``；
``ContrastiveLoss`` 的结构不需要改动（它把配对细节收敛在
``paired_infonce_loss`` 一个函数里）。这一差异在
``docs/implementation_notes.md`` 也有记录。
